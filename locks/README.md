...
## SQL, Protobuf, and gRPC projections

The peer contract has executable downstream projections generated by
`node tools/generate-contract-projections.mjs`:

- `generated/sql/from-typespec.sql` and `generated/sql/from-json-schema.sql`
  are generated independently and must converge byte-for-byte;
- `generated/protobuf/agent_pontifex/locks/v1/locks.proto` uses the append-only
  identity ledger in `projection/protobuf.lock.json`;
- the Proto contains the read-only
  `agent_pontifex.locks.v1.LockCatalogService/GetLockCatalog` gRPC method;
- `generated/sql/catalog-seed.sql` materializes the reviewed catalog only in
  disposable/shadow databases; production DDL and data changes remain owned by
  the serialized migrator;
- `generated/manifest.json` and `generated/grpc/manifest.json` bind every output
  to exact authority, catalog, lock-ledger, and Proto digests.

Run:

```sh
node tools/generate-contract-projections.mjs
node tools/generate-contract-projections.mjs --check
